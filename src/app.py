...
# On s'assure que preprocess.py est dans le meme dossier
try:
    from preprocess import filter_data
except ImportError:
    print("ERREUR: Le fichier 'preprocess.py' est introuvable. Veuillez le placer dans le meme dossier que app.py.")
    # On definit une fonction de substitution pour eviter que l'application ne plante
...
# CORRECTED: Construct an absolute path to the data file.
# This is a more robust way to handle file paths in deployed applications.
try:
    ...
    print(f"ERREUR: Le fichier CSV est introuvable au chemin attendu.")
...
# Definition des couleurs pour les graphiques
colors = {
    'positive': '#f97316',
    'negative': '#0ea5e9',
    'background': '#f0f4f8',
...
# ==============================================================================
# 2. Fonctions de creation de graphiques
# ==============================================================================

def create_age_gender_chart(df):
    """Cree le graphique de distribution par age et par sexe."""
    if df.empty:
        return go.Figure().update_layout(title='Aucune donnee a afficher')
    ...
    fig = px.bar(
        ...
        labels={'count': 'Nombre de patients', 'age_group': 'Groupe d\'age', 'gender': 'Sexe', 'outcome': 'Resultat'},
        color_discrete_map={'Positive': colors['positive'], 'Negative': colors['negative']},
        title='Distribution des resultats par age et par sexe'
    )
...
def create_blood_pressure_chart(df):
    """Cree le nuage de points pour la pression sanguine."""
    if df.empty:
        return go.Figure().update_layout(title='Aucune donnee a afficher')
    fig = px.scatter(
        df, x='systolic_bp', y='diastolic_bp', color='outcome',
        color_discrete_map={'Positive': colors['positive'], 'Negative': colors['negative']},
        labels={'systolic_bp': 'Pression systolique (mmHg)', 'diastolic_bp': 'Pression diastolique (mmHg)'},
        title='Pression systolique vs. diastolique'
    )
    fig.update_layout(plot_bgcolor=colors['card'], paper_bgcolor=colors['card'], font_color=colors['text'])
    return fig

def create_biomarkers_chart(df):
    if df.empty: return go.Figure().update_layout(title='Aucune donnee a afficher')
    ...
    fig = px.bar(biomarkers_melted, x='biomarker', y='median_value', color='outcome', barmode='group',
        labels={'median_value': 'Niveau median (echelle log)', 'biomarker': 'Biomarqueur', 'outcome': 'Resultat'},
        color_discrete_map={'Positive': colors['positive'], 'Negative': colors['negative']}, title='Niveaux des biomarqueurs par resultat', log_y=True)
    fig.update_layout(plot_bgcolor=colors['card'], paper_bgcolor=colors['card'], font_color=colors['text'])
    return fig

# ==============================================================================
# 3. MISE EN PAGE DE L'APPLICATION DASH
# ==============================================================================
...
app.layout = html.Div(className='bg-slate-100 font-sans', children=[
    ...
    html.Header(className='bg-slate-800 text-white p-6 text-center shadow-lg', children=[
        html.H1('Exploration des facteurs de risque d\'infarctus', className='text-3xl md:text-4xl font-bold mb-2'),
        html.P('Analyse interactive des indicateurs medicaux et de leur correlation avec les resultats cardiaques.', className='text-slate-300 max-w-3xl mx-auto')
    ]),
    ...
    html.Main(className='container mx-auto p-4 md:p-8', children=[
        ...
        html.Div(className='bg-white p-6 rounded-lg shadow-md mb-8', children=[
            html.H2('Controles de filtrage', className='text-xl font-bold mb-4 text-slate-700'),
            html.Div(className='grid grid-cols-1 md:grid-cols-2 gap-6', children=[
                html.Div([
                    html.Label('Tranche d\'age', className='font-semibold text-gray-700'),
                ...
                html.Div([
                    html.Label('Sexe', className='font-semibold text-gray-700'),
                    dcc.Dropdown(id='gender-dropdown',
                        options=[{'label': 'Tous', 'value': 'All'}, {'label': 'Homme', 'value': 'Male'}, {'label': 'Femme', 'value': 'Female'}],
        ...
        html.Section(id='demographics', className='bg-white p-6 rounded-lg shadow-md mb-8', children=[
            html.H2('Section 1: Demographie et signes vitaux', className='text-2xl font-bold mb-4 text-slate-700'),
        ...
        html.Section(id='biomarkers', className='bg-white p-6 rounded-lg shadow-md mb-8', children=[
            html.H2('Section 2: L\'impact des biomarqueurs cardiaques', className='text-2xl font-bold mb-4 text-slate-700'),
            dcc.Graph(id='biomarkers-chart'),
        ]),
    ]),
    
    html.Footer(className='text-center p-6 bg-slate-800 text-slate-400 text-sm', children=[
        html.P("INF8808E - Projet de visualisation de donnees | Analyse d'infarctus")
    ])
])

# ==============================================================================
# 4. CALLBACK PRINCIPAL (LE COEUR DE L'INTERACTIVITE)
# ==============================================================================
@app.callback(
...
def update_graphs(age_range, gender_value):
    """
    Ce callback principal met a jour tous les graphiques en se basant sur les filtres.
    Il utilise la fonction filter_data de preprocess.py.
    """
...
# ==============================================================================
# 5. Point d'entree pour lancer l'application
# ==============================================================================
